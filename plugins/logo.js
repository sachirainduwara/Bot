...
for (const [effectName, effectInfo] of Object.entries(effects)) {
    cmd({
        ...
        try {
            if (!args.length) {
                return await conn.sendMessage(from, {
                    text: `╭━━━〔 *✨ LOGO GENERATOR ✨* 〕━━━\n` +
                          ...
                          `┃ 📌 *Example:* \`${prefix + command} Your Text\`\n` +
                          `┃\n` +
                          `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
            ...
            if (!result.success) {
                await conn.sendMessage(from, { react: { text: "❌", key: mek.key } });
                return reply(`❌ *Failed to create logo:* ${result.error} ⚠️`);
            }

            const formattedName = effectName.charAt(0).toUpperCase() + effectName.slice(1);
            const logoCaption = `╭━━━〔 *🎨 LOGO MAKER ⚡* 〕━━━\n` +
                                `┃\n` +
...
cmd({
    ...
    try {
        ...
        if (results.length > 0) {
            let searchMsg = `╭━━━〔 *🔍 SEARCH RESULTS (${results.length})* 〕━━━\n` +
                            `┃\n` +
                            results.join('\n') + `\n` +
                            `┃\n` +
                            `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
...
cmd({
    ...
    try {
        if (!args.length) {
            return await conn.sendMessage(from, {
                text: `╭━━━〔 *🎲 RANDOM LOGO MAKER* 〕━━━\n` +
                      `┃\n` +
                      `┃ ⚠️ *Please provide text!*\n` +
                      `┃ 📌 *Example:* \`${prefix + command} My Text\`\n` +
                      `┃\n` +
                      `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                      `> *⚡ Powered by SACHIYA-MD 💫*`
            }, { quoted: mek });
        ...
        if (!result.success) {
            await conn.sendMessage(from, { react: { text: "❌", key: mek.key } });
            return reply(`❌ *Failed:* ${result.error} ⚠️`);
        }

        const formattedRandom = randomEffect.charAt(0).toUpperCase() + randomEffect.slice(1);
        const randomCaption = `╭━━━〔 *🎲 RANDOM LOGO ⚡* 〕━━━\n` +
                              `┃\n` +
                              `┃ 📌 *Effect:* ${formattedRandom}\n` +
                              `┃ ✍️ *Text:* ${text}\n` +
...
cmd({
    ...
    try {
        if (args.length < 2) {
            return await conn.sendMessage(from, {
                text: `╭━━━〔 *🔄 BATCH LOGO MAKER* 〕━━━\n` +
                      `┃\n` +
                      `┃ ⚠️ *Usage:* \`${prefix + command} [effect1,effect2,...] [text]\`\n` +
                      `┃ 📌 *Example:* \`${prefix + command} naruto,dragonball,neon Uzumaki\`\n` +
                      `┃\n` +
                      `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                      `> *⚡ Powered by SACHIYA-MD 💫*`
            }, { quoted: mek });
        ...
        if (validEffects.length === 0) {
            return reply(`❌ *No valid effects found. Invalid:* ${invalidEffects.join(', ')} ⚠️`);
        }
        ...
        for (const effect of validEffects) {
            try {
                ...
                if (result.success) {
                    ...
                    const batchCaption = `╭━━━〔 *🔄 BATCH LOGO (${createdLogos.length}/${validEffects.length})* 〕━━━\n` +
                                         `┃\n` +
                                         `┃ 📌 *Effect:* ${effect}\n` +
                                         `┃ ✍️ *Text:* ${text}\n` +
        ...
        if (createdLogos.length === 0) {
            await conn.sendMessage(from, { react: { text: "❌", key: mek.key } });
            return reply("❌ *Failed to create any logos.* ⚠️");
        }
        
        await conn.sendMessage(from, { react: { text: "✅", key: mek.key } });
        await reply(`✅ *Successfully created ${createdLogos.length}/${validEffects.length} logos!* 🎉`);
...
cmd({
    ...
    try {
        if (!args.length) {
            return await conn.sendMessage(from, {
                text: `╭━━━〔 *ℹ️ LOGO INFO HELP* 〕━━━\n` +
                      `┃\n` +
                      `┃ ⚠️ *Please specify an effect!*\n` +
                      `┃ 📌 *Example:* \`${prefix + command} naruto\`\n` +
                      `┃ 📋 *Tip:* Use \`.logo list\` to see all effects.\n` +
                      `┃\n` +
                      `╰━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
                      `> *⚡ Powered by SACHIYA-MD 💫*`
            }, { quoted: mek });
        ...
        if (!effects[effect]) {
            return reply(`❌ *Effect "${effect}" not found.*\nUse \`.logo list\` to see all effects. ⚠️`);
        }
        
        const info = effects[effect];
        const formattedEffectName = effect.charAt(0).toUpperCase() + effect.slice(1);
        
        const message = `╭━━━〔 *ℹ️ EFFECT INFO: ${formattedEffectName}* 〕━━━\n` +
                        `┃\n` +
