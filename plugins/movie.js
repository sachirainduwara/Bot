...
cmd({
  ...
  if (!q) return reply(`*ð¬ Movie Search Plugin*\nUsage: movie_name\nExample: movie avengers`);
  reply("*ð Searching for movies...*");
  const searchResults = await searchMovies(q);
  if (!searchResults.length) return reply("*â No movies found!*");
  pendingSearch[sender] = { results: searchResults, timestamp: Date.now() };
  let text = "*ð¬ Search Results:*\n";
  searchResults.forEach((m, i) => {
    text += `*${i+1}.* ${m.title}\n   ð Language: ${m.language}\n   ð Quality: ${m.quality}\n   ðï¸ Format: ${m.qty}\n`;
  });
...
cmd({
  ...
  await danuwa.sendMessage(from, { react: { text: "â", key: m.key } });
  const index = parseInt(body.trim()) - 1;
  const selected = pendingSearch[sender].results[index];
  delete pendingSearch[sender];
  const metadata = await getMovieMetadata(selected.movieUrl);
  let msg = `*ð¬ ${metadata.title}*\n`;
  msg += `*ð Language:* ${metadata.language}\n*â±ï¸ Duration:* ${metadata.duration}\n*â­ IMDb:* ${metadata.imdb}\n`;
  msg += `*ð­ Genres:* ${metadata.genres.join(", ")}\n*ð¥ Directors:* ${metadata.directors.join(", ")}\n*ð Stars:* ${metadata.stars.slice(0,5).join(", ")}${metadata.stars.length>5?"...":""}\n\n`;
  msg += "*ð Fetching download links, please wait...*";
  ...
  if (!downloadLinks.length) return reply("*â No download links found (<2GB)!*");
  pendingQuality[sender] = { movie: { metadata, downloadLinks }, timestamp: Date.now() };
  let qualityMsg = "*ð¥ Available Qualities (Max 2GB):*\n";
...
cmd({
  ...
  try {
    ...
    await danuwa.sendMessage(from, {
      ...
      caption: `*ð¬ ${movie.metadata.title}*\n*ð Quality:* ${selectedLink.quality}\n*ð¾ Size:* ${selectedLink.size}\n\n*Enjoy your movie! ð¿*`
    }, { quoted: mek });
  } catch (error) {
    console.error("Send document error:", error);
    reply(`*â Failed to send movie:* ${error.message || "Unknown error"}`);
  }
});
