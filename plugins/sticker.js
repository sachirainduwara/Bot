...
cmd(
  ...
  async (sachiya, mek, m, { from, quoted, q, reply, pushname }) => {
    try {
      ...
      if (m?.type === "imageMessage" || isQuotedImage) {
        reply("*⏳ Making your rounded sticker... Please wait!* 🔄");
        ...
        await sachiya.sendMessage(from, { sticker: buf }, { quoted: mek });
        return reply("*✅ Rounded sticker created successfully!* ✨");
      }

      if (isQuotedSticker) {
        reply("*⏳ Re-shaping sticker... Please wait!* 🔄");
        ...
        const buf = await st.toBuffer();
        await sachiya.sendMessage(from, { sticker: buf }, { quoted: mek });
        return reply("*✅ Sticker rounded successfully!* ✨");
      }

      if (q && q.trim().length > 0) {
        reply("*⏳ Creating Sinhala/Text sticker... Please wait!* 🔄");
        ...
        const buf = await st.toBuffer();
        await sachiya.sendMessage(from, { sticker: buf }, { quoted: mek });
        return reply("*✅ Sinhala/Text sticker created successfully!* ✨");
      }

      return reply(
        `╭━━━〔 *✨ SACHIYA-MD STICKER ✨* 〕━━━\n` +
        `┃\n` +
        `┃ ⚠️ *Reply to image/sticker or type text after \`.sticker\`!*\n` +
        `┃\n` +
        `┃ *Options:*\n` +
        `┃ • \`--circle\`\n` +
        `┃ • \`--rounded\`\n` +
      ...
      console.error("❌ SACHIYA-MD Sticker Error:", e);
      return reply("*❌ Failed to generate sticker. Check logs for details.* ⚠️");
    }
  }
);
