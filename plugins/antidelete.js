...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    if (!message.key.fromMe && !isOwner) {
        return sock.sendMessage(chatId, { text: '⚠️ *මෙම විධානය භාවිතා කළ හැක්කේ බොට් හිමිකරුට (Owner) පමණි!*' }, { quoted: message });
    }
    ...
    if (match === 'on') {
        ...
        return sock.sendMessage(chatId, { text: '⚠️ *වැරදි විධානයකි! භාවිතය සඳහා .antidelete ලෙස යොදන්න.*' }, { quoted: message });
    }

    saveAntideleteConfig(config);
    return sock.sendMessage(chatId, { text: `✨ *Antidelete System successfully ${match === 'on' ? 'Enabled 🟢' : 'Disabled 🔴'}!*` }, { quoted: message });
}
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        let text = `╭━━━〔 *🗑️ SACHIYA-MD ANTIDELETE* 〕━━━\n` +
                   ...
                   `┃ 📱 *Number:* wa.me/${senderNumberFormat(sender)}\n` +
                   `┃ 🕒 *Time:* ${time}\n`;
        ...
        if (original.mediaType && fs.existsSync(original.mediaPath)) {
            ...
            try {
                ...
                await sock.sendMessage(ownerNumber, {
                    text: `⚠️ *Media යැවීමේදී දෝෂයක් ඇති විය: ${err.message}*`
                });
            }
...
function senderNumberFormat(jid) {
