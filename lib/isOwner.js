...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    // settings වෙනුවට config.js එකට අදාළව owner number එක ලබා ගැනීම
    const ownerNum = config.OWNER_NUMBER || config.owner || '';
    const ownerJid = ownerNum + "@s.whatsapp.net";
    const ownerNumberClean = ownerNum.split(':')[0].split('@')[0];
